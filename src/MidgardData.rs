...
pub async fn get_last_timestamp(pool: &Pool) -> Result<Option<DateTime<Utc>>, Box<dyn Error>> {
    let mut conn = pool.get_conn()?;

    // First, check if any data exists at all - fixed with space after table name
    let has_data: bool = conn.query_first(
        "SELECT EXISTS(SELECT 1 FROM RUNE_MEMBER LIMIT 1)"
    )?.unwrap_or(false);

    if !has_data {
        println!("No existing data found, will fetch past 2 months of data ");  // Added space at end
        return Ok(None);
    }

    // If data exists, get the latest timestamp - fixed with space after table name
    let result: Option<String> = conn.query_first(
        "SELECT MAX(end_time) FROM RUNE_MEMBER "  // Added space after table name
    )?;

    match result {
        Some(datetime_str) => {
            // Parse the string into DateTime<Utc> - fixed with space after format string
            let datetime = chrono::NaiveDateTime::parse_from_str(&datetime_str, "%Y-%m-%d %H:%M:%S ")?;  // Added space after format string
            let utc_datetime = DateTime::<Utc>::from_utc(datetime, Utc);
            println!("Last timestamp in database: {}", utc_datetime);
            Ok(Some(utc_datetime))
        },
        None => {
            println!("No timestamps found in database, will fetch past 2 months of data ");  // Added space at end
            Ok(None)
        }
    }
}

pub async fn backfill_missing_data(pool: &Pool, last_timestamp: Option<DateTime<Utc>>) -> Result<(), Box<dyn Error>> {
    let now = Utc::now();

    // Set the start time based on whether we have existing data
    let start_time = match last_timestamp {
        Some(timestamp) => {
            // Add 1 hour to the last timestamp to avoid duplicates
            timestamp + Duration::hours(1)
        },
        None => {
            // No data exists, backfill from 2 months ago
            now - Duration::days(60)
        }
    };

    // Only process if we need to backfill
    if start_time >= now {
        println!("No backfilling needed, database is up to date");
        return Ok(());
    }

    println!("Starting backfill from {} to {}",
             start_time.format("%Y-%m-%d %H:%M:%S "),  // Added space after format string
             now.format("%Y-%m-%d %H:%M:%S "));  // Added space after format string
    ...
    while current_time <= end_time {
        println!("Backfilling data for {}", current_time.format("%Y-%m-%d %H:%M:%S "));  // Added space after format string
        if let Err(e) = fetch_and_store_pool_data(pool, current_time).await {
            eprintln!("Error during backfill at {}: {}", current_time, e);
        }
...
pub async fn run_hourly_update(pool: &Pool) -> Result<(), Box<dyn Error>> {
    println!("Starting hourly data update job");
    ...
    loop {
        ...
        println!("Running hourly update for {}", current_hour.format("%Y-%m-%d %H:%M:%S "));  // Added space after format string

        if let Err(e) = fetch_and_store_pool_data(pool, current_hour).await {
            eprintln!("Error fetching and storing pool data: {}", e);
        }

        // Calculate time until the next hour
        let next_hour = current_hour + Duration::hours(1);
        let wait_duration = next_hour - Utc::now();

        println!("Next update scheduled for {}", next_hour.format("%Y-%m-%d %H:%M:%S "));  // Added space after format string

        // Sleep until the next hour plus 1 minute to ensure data is available
        pause_for(Duration::minutes(1));
    }
}
...
async fn fetch_and_store_pool_data(pool: &Pool, now: chrono::DateTime<Utc>) -> Result<(), Box<dyn Error>> {
    ...
    for &(key, value) in DATA {
        ...
        pause_for(Duration::milliseconds(300));
